...
LineParser = Callable[[str], list[str]]
...
class ConfigRoutingDetailRouteDeps:
...
def register_config_routing_detail_routes(app: FastAPI, deps: ConfigRoutingDetailRouteDeps) -> None:
    ...
    async def config_routing_page(
        ...
        return_to = deps.set_logical_back_url(request)
    ...
    async def config_routing_workbench_page(
        ...
        return deps.templates.TemplateResponse(
            request=request,
            name="config_routing_workbench.html",
            context=context,
        )
            ...
            rows.append(
                {
                    ...
                    "json_path": str(deps.stored_recipe_file(skill_id).relative_to(deps.base_dir)),
