...
def test_capability_executor_bindings_cover_expected_pairs() -> None:
    ...
    assert ("google_calendar", "calendar_read") in bindings
...
def test_build_capability_detail_lines_uses_catalog_detail_metadata() -> None:
    ...
    calendar_details = build_capability_detail_lines(
        ActionPlan(capability="calendar_read", connection_kind="google_calendar", connection_ref="primary-calendar", path="tomorrow"),
        lambda kind: kind.upper(),
    )
    ...
    assert calendar_details[-1] == "Zeitraum: tomorrow"
