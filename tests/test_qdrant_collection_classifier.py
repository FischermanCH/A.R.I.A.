...
def test_recipe_experience_collection_is_system_learning_memory() -> None:
    row = classify_qdrant_collection("aria_recipe_experience_fischerman")

    assert row.kind == "recipe_experience"
    assert row.group == "system"
    assert row.is_system is True
    assert row.is_user_memory is False
    assert is_recipe_experience_qdrant_collection("aria_recipe_experience_fischerman") is True


def test_future_aria_collection_remains_visible_as_system() -> None:
    row = classify_qdrant_collection("aria_future_signal_fischerman")
...
def test_user_memory_collections_are_not_generic_system_collections() -> None:
    assert classify_qdrant_collection("aria_docs_fischerman_manuals").kind == "document"
    assert classify_qdrant_collection("aria_facts_fischerman").kind == "fact"
    assert classify_qdrant_collection("aria_preferences_fischerman").kind == "preference"
    assert classify_qdrant_collection("aria_sessions_fischerman_260513").kind == "session"
    assert classify_qdrant_collection("aria_context-mem_fischerman").kind == "knowledge"

    for name in (
        "aria_docs_fischerman_manuals",
        "aria_facts_fischerman",
        "aria_preferences_fischerman",
        "aria_sessions_fischerman_260513",
...
def test_notes_and_routing_helpers_keep_scope_boundaries() -> None:
    assert is_notes_qdrant_collection("aria_notes_fischerman", username="fischerman") is True
    assert is_notes_qdrant_collection("aria_notes_other", username="fischerman") is False
